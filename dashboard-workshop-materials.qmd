...
4.  We might want our dashboard to be able to filter down to one of our provinces in the dataset - lets test out some of these examples above and select the most appropriate option for this.
...
Lets now create the skeleton of our dashboard. Firstly what we want to be able to do is allow user to select a **Province** from a dropdown list of all the unique values in our dataset.

We’ll start by updating the **UI** to add a control widget `selectInput()` that will allow the user to select a Province from our dataset.
...
-   How to connect your `**Input()` to your dataset to generate outputs.
...
So for example, our app has a widget named `"province_select"` .The values of `"province_select"` will be saved in `input` as `input$province_select`.
...
Each entry to `output$ <-` should contain the output of one of Shiny’s `render*` functions. These functions capture an R expression and do some light pre-processing on the expression. Use the `render*` function that corresponds to the type of reactive object you are making (Pairs are listed in the table above).
...
Now that we’ve built a simple reactive and styled our app, let’s level up by introducing **addiditional output types** and features so that we can convey more information than what we have currentlly presented in our app.
...
At this staage we have learnt some great skills on building Shiny dashboards but now is a good time to take a step back and this about our end-user who we are designing this application for - a really key part of building a good dashboard.

Let's take a step back and evolve the app to reflect a more **realistic reporting structure** often seen in Ministry of Health surveillance dashboards.

**Why make this change?**

-   Many surveillance dashboards begin at the **national level**, providing a quick high-level summary of the country’s malaria indicators.

-   The user can then **drill down** into province-specific trends when needed.

-   This allows stakeholders to get both a big-picture view and explore granular data.

**We'll evolve our app to allow:**

-   Default view = National summary (all provinces combined)

-   User can toggle between National or Province level using a **radio button**

-   If Province is selected, a dropdown appears allowing the user to pick a Province.

-   All plots, value boxes, and outputs automatically adjust based on this selection.

Let's first update the **sidebar UI** to allow the user to select the reporting level, update our description and .

Update your `sidebar = sidebar()` with:
...
sidebar = sidebar(
  ...
  radioButtons(
    inputId = "reporting_level", # ID for crosstalk in the app
    label = "Select reporting level:", # Titlefor user side
    choices = c("National", "Province"), # Options of selection 
...
::: callout-note
The `uiOutput("province_selector")` creates a placeholder — we'll dynamically insert the Province dropdown *only* when Province is selected.
:::

Next, we need to add the logic to conditionally display the Province dropdown based on the selected reporting level. Add this to the top of your `server()` function:
...
This ensures the Province dropdown only appears when appropriate. We’re giving users a cleaner, more intuitive interface!

::: {layout-ncol="2"}
![](images/user-inputs-national-on.PNG)

![](images/user-inputs-national-off.PNG)
:::

Now we need to adjust our reactive data filter to reflect both National and Province level selections.

Replace your existing `province_data()` reactive with this:
...
Now replace all server-side value box calculations to use `filtered_data()` instead of `province_data()`, add a condidtional statement to only show our `validate()` messages when the dashboard is in "Province" level view and make sure the numerical values are formatted nicely.
...
And update the call to `value_box()` so that now our data will always be displayed with numeric comma style presentation for easier reading.
...
#-Value Box dynamic number based on TPR toggle-----------------------------------------------------
  output$value_boxes <- renderUI({
    ...
    boxes <- list(
      value_box(
        title = "Total Cases Reported",
        value = textOutput(scales::comma("total_cases_box")), # use scales::comma to format the number with commas
        showcase = bsicons::bs_icon("journal-medical"),
        showcase_layout = "top right", 
        theme = "secondary"
      ),
      value_box(
        title = "Total Tests Reported", 
        value = textOutput(scales::comma("total_tests_box")), # use scales::comma to format the number with commas
        showcase = bsicons::bs_icon("journal-medical"), 
        showcase_layout = "top right", 
        theme = "secondary" 
      )
    ...
    if (isTRUE(input$include_tpr)) {
      boxes <- append(boxes, list(
        value_box(
          title = "Average TPR (%)",
          value = textOutput(scales::comma("tpr_value")), # use scales::comma to format the number with commas
          showcase = bsicons::bs_icon("percent"),
          showcase_layout = "top right",
          theme = "secondary"
        )
...
![](images/national-value-boxes.PNG)

Update the plotting code to also use `filtered_data()` instead of `province_data()`, add a condidtional statement to only show our `validate()` messages when the dashboard is in "Province" level view and update the title text to be informative at the National level.
...
And in the `ui()` lets reset our default selection of plot variables to set the default to fill the plot first with case data
      ...
      layout_sidebar(
        ...
        sidebar = list(
          ...
          checkboxGroupInput(
            ...
            selected = "malaria_cases" # set the default to cases
          ),
          # condiditionally render TPR checkbox
          uiOutput("tpr_checkbox_ui")
        ),
...
![](images/national-default-plot.PNG)

### Stretch Exercise

🎯 Your Task - add a new card panel below the current plots showing a time series plot of incidence per 1,000 people. Beacuse the dataset does not contain population data we cannot calculate the national level monthly incidence so we will display monthly data for each province as a single line to see the range across the country.

This plot will:

-   Use data where `var_name == "malaria_cases_per_1000"`

-   Show a single line plot with one point per month for each province coloured by province

-   Appear in a new `card()` component with a header and footer

-   Appears by default when National level is selected but displays Province selection message if Province level selected.
...
![](images/national-inc.PNG)

![](images/province-inc.PNG)
:::

## Including Leaflet Maps

Now that your dashboard includes time series and summary statistics, let’s add a **spatial view** using an interactive map. This will help users visually explore **malaria burden across the country**.

We’ll use the `{leaflet}` package to create a **map** of average annual incidence per 1,000 people, and when our dashboard is in Province view we’ll highlight the **selected province** from the sidebar dropdown.
...
Add a `layout_columns()` and an extra `card` to ensure the map and the incidence plot fall on the same row of the dashboard and adjust the widths to allow more space for the line plot.
...
# incidence plot card - including map 
    layout_columns(
      ...
      card(
        ...
        card_body(plotlyOutput("inc_plot", height = "400px")),
        card_footer("Data source: DHIS2 export")
      ), 
      card(
        height = 400,
        ...
        card_body(leafletOutput("inc_map", height = "400px")),
...
![](images/map-and-inc-national.PNG)
...
-   Aggregates total cases, total tests, average TPR and average incidence by province over the year and includes the number of months that data is reported for in the province

-   Highlights the currently selected province if province level display is selected
...
# DT output 
    card(
      ...
      card_footer("Aggregated Annual Values") # card footer
    )
...
-   Select a spatial scale and filter/ process data interactively
...
::: callout-important
There are so of course so many elements we could build into our dashboards and figures that are useful ways to present our data. Here are some examples but can you thik of any more:

-   If we have addiditional years of data adding time based filtering inputs

-   An addiditonal tab used to compare locations or time periods

-   Ability to select a location based on a map input
